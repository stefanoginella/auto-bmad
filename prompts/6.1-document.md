...
3. Populate the 'Suggested Review Order' section in the pipeline report. This is a navigable review trail for human reviewers:
...
4. Add a '## Human Review Required' section BEFORE the Auto-bmad Completion section. This consolidates all items requiring human attention from this story. It has four subsections:
...
6. Add an '## Auto-bmad Pipeline Artifacts' section at the very end of the story file. This is a reference index ONLY — do not duplicate content already in the Dev Agent Record, Change Log, or the artifacts themselves. List each artifact with a one-line outcome (pass/fail/N findings). Skip any that don't exist:
   - Pipeline report: {{STORY_ARTIFACTS}}/{{STORY_SHORT_ID}}-6.1-pipeline-report.md
   - Spec triage: {{STORY_ARTIFACTS}}/{{STORY_SHORT_ID}}-1.3-spec-triage.md
   - Code acceptance audit: {{STORY_ARTIFACTS}}/{{STORY_SHORT_ID}}-3.2-code-acceptance.md
   - Code triage: {{STORY_ARTIFACTS}}/{{STORY_SHORT_ID}}-3.3-code-triage.md